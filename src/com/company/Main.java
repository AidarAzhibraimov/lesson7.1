...
public class Main {
	...
	Hero [] heroes = {new Warrior(), new Magic(), new Medic()};
        for (int i = 0; i <heroes.length; i++) {
            if (heroes[i] instanceof Hero) ((Hero) heroes[i]).applySuperAbility();
        }
    }
}
/*#ДЗУрок7:
Создать интерфейс HavingSuperAbility (имеющий суперспособность) добавить в него
 1 абстрактный метод applySuperAbility(String superAbilityType);
